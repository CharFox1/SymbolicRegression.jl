...
function multiply_powers(
    eqn::T
)::Tuple{SYMBOLIC_UTILS_TYPES,Bool} where {T<:Union{<:Number,SymbolicUtils.Sym{<:Number}}}
    return eqn, true
end

function multiply_powers(
    eqn::T, op::F
)::Tuple{SYMBOLIC_UTILS_TYPES,Bool} where {F,T<:SymbolicUtils.Term{<:Number}}
...
function multiply_powers(
    eqn::T
)::Tuple{SYMBOLIC_UTILS_TYPES,Bool} where {T<:SymbolicUtils.Term{<:Number}}
...
function get_simplifier(binops::A, unaops::B) where {A,B}
    ...
    serial_polynormal_simplifier = If(
        istree, Fixpoint(Chain((polynormalize, Fixpoint(default_simplifier()))))
    )
    return serial_polynormal_simplifier
end

function custom_simplify(
    init_eqn::T, options::Options
)::Tuple{SYMBOLIC_UTILS_TYPES,Bool} where {T<:SYMBOLIC_UTILS_TYPES}
