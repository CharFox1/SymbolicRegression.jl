...
@from "Utils.jl" import debug,
    debug_inline, is_anonymous_function, recursive_merge, next_worker, @sr_spawner
...
@from "MutationFunctions.jl" import genRandomTree,
    genRandomTreeFixedSize, randomNode, randomNodeAndParent, crossoverTrees
...
function _EquationSearch(
    ...
    # check to make sure multithreading if using sym_constraints
    #@assert ConcurrencyType in [SRThreaded, SRSerial] || options.sym_constraints === nothing

    # try to pass python libary through options
    #options.pyLibs = Dict{String, PyObject}()
    ...
    #dill = pyimport("dill")
    #sympy = PyNULL()
    #copy!(sympy, pyimport_conda("sympy", "sympy"))
    ...
    frequencyComplexities = [ones(T, actualMaxsize) for i in 1:nout]
        ...
        if population_ready
            ...
            open(hofFile, "w") do io
                ...
                for member in dominating
                    adjusted_score =
                        (member.score - countNodes(member.tree) * options.parsimony) *
                        baselineMSE
                    println(
                        io,
                        "$(countNodes(member.tree))|$(adjusted_score)|$(stringTree(member.tree, options, varMap=dataset.varMap))",
                    )
                end
            end
        ...
        if options.earlyStopCondition !== nothing
            ...
            for j in 1:nout
                ...
                if length(dominating) == 0
                    all_below = false
                elseif !any([
                    options.earlyStopCondition(member.score, member.complexity) for
