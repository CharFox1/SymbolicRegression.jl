...
const SYMBOLIC_UTILS_TYPES = Union{
    <:Number,SymbolicUtils.Sym{<:Number},SymbolicUtils.Term{<:Number}
}
...
function node_to_symbolic(
    ...
    elseif tree.degree == 1
        ...
        if (op in (+, -, *, /)) || evaluate_functions
            ...
            if index_functions
                dummy_op = SymbolicUtils.Sym{
                    (SymbolicUtils.FnType){Tuple{Number,Number},Real}
                }(
...
function node_to_symbolic_safe(
    tree::Node,
    options::Options;
    varMap::Union{Array{String,1},Nothing}=nothing,
    evaluate_functions::Bool=false,
    ...
    elseif tree.degree == 1
        left_side, complete = node_to_symbolic_safe(
            tree.l,
            options;
            varMap=varMap,
            evaluate_functions=evaluate_functions,
        ...
        @return_on_false complete Inf
        @return_on_false isgood(left_side) Inf
        right_side, complete2 = node_to_symbolic_safe(
            tree.r,
            options;
            varMap=varMap,
            evaluate_functions=evaluate_functions,
...
function _multiarg_split(
    op_idx::Int, eqn::Array{Any,1}, options::Options, varMap::Union{Array{String,1},Nothing}
)::Node
    if length(eqn) == 2
        return Node(
            op_idx,
...
function unaop_to_index(op::F, options::Options)::Int where {F<:SymbolicUtils.Sym}
    # In format _unaop1
    return parse(Int, string(op.name)[7:end])
end
...
function unaop_to_index(op::F, options::Options)::Int where {F<:Function}
    ...
    return error(
        "Operator $(op) in simplified expression not found in options $(options.unaops)!"
    )
end
...
function binop_to_index(op::F, options::Options)::Int where {F<:Function}
    ...
    return error(
        "Operator $(op) in simplified expression not found in options $(options.binops)!"
    )
end
