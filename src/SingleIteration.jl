...
function SRCycle(
    ...
    for temperature in 1:size(allT, 1)
        ...
        if verbosity > 0 && (temperature % verbosity == 0) # TODO: Remove this
            ...
            bestCurScoreIdx = argmin([
                bestPops.members[member].score for member in 1:(bestPops.n)
            ])
            ...
            debug(
                ...
                " is the score for ",
                stringTree(
                    bestPops.members[bestCurScoreIdx].tree, options; varMap=dataset.varMap
                ),
            )
        end
    end
...
function OptimizeAndSimplifyPopulation(
    ...
    @inbounds @simd for j in 1:(pop.n)
        ...
        if options.use_symbolic_utils
            pop.members[j].tree = simplifyWithSymbolicUtils(
                pop.members[j].tree, options, curmaxsize
            )
        end
